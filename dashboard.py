...
with st.sidebar:
    ...
    st.code("sudo python3 nids.py -i wlp0s20f3", language="bash")
...
if not os.path.exists(DB_PATH):
    ...
    st.code("sudo python3 nids.py -i wlp0s20f3")
    st.stop()
...
query = "SELECT * FROM alerts" + (f" WHERE severity='{severity_filter}'" if severity_filter != "ALL" else "") + " ORDER BY id DESC LIMIT 30"
...
if alerts_df.empty:
    ...
    for _, row in alerts_df.iterrows():
        ...
        st.markdown(f'<div class="{sev}"><b>{row["severity"]}</b> | PORT SCAN | Source: <b>{row["src_ip"]}</b> | Ports Hit: <b>{row["port_count"]}</b><br>Ports: {ports[:15]}<br><span style="color:#6e7681;">{row["timestamp"]}</span></div>', unsafe_allow_html=True)
